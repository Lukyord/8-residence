...
jQuery(function ($) {
    if ($(".blurred-media").length) {
        $(".blurred-media").each(function () {
            ...
            const media = blurredImageDiv.find("img, video, iframe");
            ...
            if (media.length) {
                ...
                console.log(
                    "No img, video, or iframe element found inside .blurred-media"
                );
            }
        });
    }
...
jQuery(function ($) {
    setTimeout(function () {
        ...
        if (window.location.hash) {
            const targetOffset =
                $(window.location.hash).offset().top - _windowHeight;
...
/* HEADER&FOOTER */
jQuery(function ($) {
    var indexMarqueeWrapper = $(".index-marquee-wrapper");

    //CHECK IF USER HAS SCROLLED PAST LIMARQUEE
    function checkIfScrolledPastMarquee() {
        if (!indexMarqueeWrapper.length) {
            $("html").addClass("scrolled-past-marquee");
            return;
        }

        if (
            $(window).scrollTop() >
            indexMarqueeWrapper.offset().top + indexMarqueeWrapper.height()
        ) {
            $("html").addClass("scrolled-past-marquee");
            $("header .button").removeClass("dark-bg");
            $("header .button").addClass("light-bg");
        } else {
            $("html").removeClass("scrolled-past-marquee");
            $("header .button").removeClass("light-bg");
            $("header .button").addClass("dark-bg");
        }
    }

    $(window).on("load scroll", checkIfScrolledPastMarquee);
});
